...
define exec
	docker run -it --rm --cpuset-cpus 0 --cpu-quota 20000 -w /work -v $(PROJECT_ROOT):/work debian:gcc $(BINARY_PATH) ${1}
endef
...
run: ## docker run
	docker run -it --rm --cpuset-cpus 0 --cap-add=SYS_PTRACE --security-opt="seccomp=unconfined" -w /work -v $(PROJECT_ROOT):/work debian:gcc /bin/bash || true
...
lib: clean ## compile lib
	...
	docker run --rm -w /work -v $(PROJECT_ROOT)/lib:/work debian:gcc ar -r libapue.a error.o io.o process.o
